...
describe('reviewRepository engine', () => {
  ...
  it('should review clean repository and return zero findings', async () => {
    ...
    expect(result.summary.findingsBySource).toEqual({ eslint: 0, complexity: 0 });
    expect(result.findings).toHaveLength(0);
    expect(result.failures).toHaveLength(0);
  });

  it('should combine and aggregate findings from ESLint and complexity analyzers', async () => {
    await fs.mkdir(path.join(tempDir, 'src'), { recursive: true });
    await fs.writeFile(
